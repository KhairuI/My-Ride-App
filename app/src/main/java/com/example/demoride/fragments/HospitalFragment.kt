...
import com.example.demoride.databinding.FragmentHospitalBinding
...
class HospitalFragment : Fragment(),OnMapReadyCallback,GoogleMap.OnMarkerClickListener {
        ...
        if(PermissionUtils.checkPermission(requireContext())){
            ...
            fusedLocationProviderClient.lastLocation.addOnCompleteListener {
                ...
                if(location != null){
                   ...
                   hospitalApiCall(makeQuery(location))
                }
            }
    ...
    private fun hospitalApiCall(url: String) {
    ...
    private fun setHospital(result: List<ResultR>?) {
        ...
        for(i in iterator!!){
            ...
            mMap.addMarker(MarkerOptions().position(latLng).icon(BitmapDescriptorFactory.fromResource(R.drawable.hospital_icon)).snippet(i.vicinity)
                .title(i.name))
        }
    }
    ...
    private fun makeQuery(location: Location):String{
        ...
        return "maps/api/place/nearbysearch/json?location=${place}&radius=5000&type=hospital&key=map_api_key(need paid api key in this case)"
    }
