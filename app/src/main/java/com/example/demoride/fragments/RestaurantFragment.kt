...
import com.example.demoride.databinding.FragmentRestaurantBinding
...
class RestaurantFragment : Fragment(),OnMapReadyCallback,GoogleMap.OnMarkerClickListener {
        ...
        if(PermissionUtils.checkPermission(requireContext())){
            ...
            fusedLocationProviderClient.lastLocation.addOnCompleteListener {
                ...
                if(location != null){
                    ...
                    restaurantApiCall(makeQuery(location))
                }
            }
        }
    }

    private fun restaurantApiCall(url: String) {
    ...
    private fun setRestaurant(result: List<ResultR>?) {
        val iterator = result?.listIterator()
        for(i in iterator!!){
            val lat= i.geometry?.location?.lat
            val lng= i.geometry?.location?.lng
            val latLng= LatLng(lat!!,lng!!)
            mMap.addMarker(MarkerOptions().position(latLng).icon(BitmapDescriptorFactory.fromResource(R.drawable.restaurant_icon)).snippet(i.vicinity)
                .title(i.name))
        }
    }
    ...
    private fun makeQuery(location:Location):String{
        ...
        return "maps/api/place/nearbysearch/json?location=${place}&radius=5000&type=restaurant&key=map_api_key(need paid api key in this case)"
    }
